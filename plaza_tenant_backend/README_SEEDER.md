...
| `tenant_fifocicil` | **Edge Case (FIFO Partial Payment)** | `password123` | 3 tagihan menunggak, dibayar Rp 750.000 → FIFO melunasi tagihan 1, menyicil tagihan 2 (`Dicicil`), tagihan 3 (`Belum Bayar`) |
...
| `pembayaran` | **1.073** | ✅ Diterima, Ditolak, Cicilan FIFO, Midtrans, Sanggahan |
...
1. **FIFO Partial Payment**:
   - `tenant_fifocicil` memiliki 3 tagihan berurutan. Saat membayar Rp 750.000, tagihan tertua otomatis `Lunas`, tagihan kedua `Dicicil` (sisa Rp 250.000), tagihan ketiga `Belum Bayar`.
