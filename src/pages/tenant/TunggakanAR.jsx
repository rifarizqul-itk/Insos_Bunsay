      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px', alignItems: 'flex-start' }}>
        <div style={{ backgroundColor: '#ffffff', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border)', padding: '24px', display: 'flex', flexDirection: 'column', gap: '16px', boxShadow: '0 2px 12px rgba(139,26,26,0.08)' }}>
          <div>
            ...
            <div style={{ fontSize: '32px', fontWeight: '800', color: 'var(--orange)', marginTop: '4px' }}>Rp {totalAwal.toLocaleString('id-ID')}</div>
          </div>
          <div style={{ fontSize: '14px', borderTop: '1px solid var(--border)', paddingTop: '16px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              <strong style={{ color: 'var(--green)' }}>Rp {totalTerbayar.toLocaleString('id-ID')}</strong>
            </div>
          ...
          <button
            ...
            disabled={sisa <= 0}
            style={{
              backgroundColor: sisa > 0 ? 'var(--red)' : 'var(--text-3)',
              color: '#ffffff',
              padding: '12px',
              fontSize: '15px',
              fontWeight: '700',
            ...
            {riwayatCicilan.length === 0 ? (
              ...
              riwayatCicilan.map((cicil) => (
                <div key={cicil.ke} style={{ border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', padding: '14px', backgroundColor: 'var(--warm-gray)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <div style={{ fontWeight: '700', fontSize: '14px', color: 'var(--text)' }}>Angsuran Ke-{cicil.ke}</div>
                    <div style={{ fontSize: '13px', color: 'var(--text-3)', marginTop: '2px' }}>Diterima: {cicil.tanggal}</div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontWeight: '700', color: 'var(--text)' }}>Rp {cicil.nominal.toLocaleString('id-ID')}</div>
