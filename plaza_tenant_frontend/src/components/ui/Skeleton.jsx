// Komponen Skeleton Loader Reusable - penanda loading halus pengganti teks biasa
import React from 'react';

export function SkeletonText({ className = 'h-4 w-3/4' }) {
  return (
    <div className={`bg-warm-gray/60 animate-pulse rounded-md ${className}`} />
  );
}

export function SkeletonCard({ className = 'h-36 w-full' }) {
  return (
    <div className={`bg-white rounded-xl border border-border p-6 shadow-card ${className}`}>
...
export function SkeletonTable({ rows = 4, className = 'w-full' }) {
  return (
    <div className={`bg-white rounded-xl border border-border overflow-hidden p-4 space-y-4 shadow-card ${className}`}>
